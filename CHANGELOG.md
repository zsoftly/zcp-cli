...
- **`zcp acl rules` / `zcp acl create-rule` / `zcp acl update-rule` / `zcp acl delete-rule`** — full ACL rule management. `update-rule` replaces a rule in place (the rule ID is preserved) and `--cidr` accepts comma-separated lists (e.g. `10.30.1.0/24,10.30.2.0/24`). The rule endpoints live under `/vpcs/{vpc}/network-acl-list/{acl_list_id}/network-acl` (singular — discovered from the backend route table after `/rules`-style probes found nothing). Rules are added one per request after the ACL list exists; `create-rule` validates the live API contract (ports required for tcp/udp, ICMP type/code for icmp, allow/deny, ingress/egress) and resolves ACL names to IDs.
