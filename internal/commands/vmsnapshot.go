...
func newVMSnapshotCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zcp vm-snapshot create --vm my-vm --name my-snap --plan basic --billing-cycle monthly --project default --region yow-1 --service virtual-machine
  zcp vm-snapshot create --vm my-vm --name my-snap --plan basic --billing-cycle monthly --project default --region yow-1 --service virtual-machine --memory`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if name == "" {
